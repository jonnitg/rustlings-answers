...
// Added the Box<dyn error::Error> since the `test_with_str` has the same returning error
fn read_and_validate(
    b: &mut dyn io::BufRead,
) -> Result<PositiveNonzeroInteger, Box<dyn error::Error>> {
    let mut line = String::new();
    b.read_line(&mut line)?; // added the ?
    let num: i64 = line.trim().parse()?; // added the ?
    let answer = PositiveNonzeroInteger::new(num)?; // added the ?
    Ok(answer)
}
